...
// =====================================================================
//  THE CAPITAL JOURNEY — WEB AUDIO ENGINE (HTML5 MP3 PLAYBACK)
//  Phát 3 bài nhạc tự chọn: "journey" (nền), "quiz" (câu hỏi), "ending" (kết thúc)
// =====================================================================
const MusicEngine = (() => {
  ...
  // Đường dẫn tới các file nhạc MP3 tự chọn
  const audioFiles = {
    journey: "audio/journey.mp3",
    quiz: "audio/quiz.mp3",
    ending: "audio/ending.mp3"
  };

  // Lưu trữ đối tượng Audio cho từng theme
  const playlist = {
    journey: null,
    quiz: null,
    ending: null
  ...
  // Quản lý các bộ timer thực hiện hiệu ứng fade âm lượng
  let fadeIntervals = {
    journey: null,
    quiz: null,
    ending: null
  ...
  // Khởi tạo hoặc lấy đối tượng Audio đã tạo
  function getAudio(theme) {
    if (!playlist[theme]) {
      playlist[theme] = new Audio(audioFiles[theme]);
      playlist[theme].loop = true;
    }
    return playlist[theme];
  }

  // Hủy tiến trình fade volume đang chạy của một theme (nếu có)
  function clearFade(theme) {
    if (fadeIntervals[theme]) {
      clearInterval(fadeIntervals[theme]);
      fadeIntervals[theme] = null;
    }
  }

  // Thay đổi dần âm lượng (fade-in hoặc fade-out)
  function fadeVolume(audio, targetVolume, duration = 800, callback = null) {
    if (!audio) {
      if (callback) callback();
      return;
    }

    const themeKey = Object.keys(playlist).find(key => playlist[key] === audio);
    if (themeKey) clearFade(themeKey);

    const startVolume = audio.volume;
    const stepTime = 30; // 30ms mỗi bước
    ...
    const intervalId = setInterval(() => {
      currStep++;
      let nextVolume = startVolume + (volumeStep * currStep);
    ...
    if (themeKey) fadeIntervals[themeKey] = intervalId;
  }

  function startCurrentTheme() {
    if (!currentTheme) return;
    const audio = getAudio(currentTheme);
    audio.volume = 0;
    
    audio.play().then(() => {
      fadeVolume(audio, 0.75, 1000);
    }).catch(err => {
      console.warn("Không thể phát nhạc do chính sách trình duyệt chưa được kích hoạt bởi người dùng:", err);
    });
  }
  ...
  function switchTheme(theme) {
    ...
    const prevAudio = playlist[prevTheme];
    if (prevAudio && !prevAudio.paused) {
      // Giảm âm lượng bài cũ rồi tạm dừng
      fadeVolume(prevAudio, 0, 500, () => {
        prevAudio.pause();
      });
    }

    const nextAudio = getAudio(currentTheme);
    nextAudio.volume = 0;
    nextAudio.play().then(() => {
      fadeVolume(nextAudio, 0.75, 800);
    }).catch(err => {
      console.warn("Không thể chuyển tiếp nhạc:", err);
    });
  }
  ...
  function startMusic(theme = "journey") {
    ...
    // Dừng tất cả các bài khác để tránh đè nhạc
    for (let key in playlist) {
      if (playlist[key] && key !== theme) {
        clearFade(key);
        playlist[key].pause();
      }
    }
    ...
    audio.play().then(() => {
      ...
      console.warn("Không thể khởi động nhạc:", err);
    });
  ...
  function stopMusic() {
    isPlaying = false;
    if (currentTheme) {
      const audio = playlist[currentTheme];
  ...
  // Kích hoạt hook sau khi DOMContentLoaded
